...
int accept_net(int listener) {
	try {return accept(listener, 0, 0);}
	catch (...) {return -1;}
}
...
int send_net(int socket, char* buf, int size) {
	try {return send(socket, buf, size, 0);}
	catch (...) {return -1;}
}

int recv_net(int socket, char* buf, int size) {
	try {return recv(socket, buf, size, 0);}
	catch (...) {return -1;}
}
...
char* getPeerIp_net(int socket) {
	...
	} catch (...) {return &std::string("fail")[0];}
}
